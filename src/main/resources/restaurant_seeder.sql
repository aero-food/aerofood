...
insert into menu_category (category) values ('Entree');
insert into menu_category (category) values ('Side');
insert into menu_category (category) values ('Dessert');
insert into menu_category (category) values ('Combos');


insert into menu_item (title, description, price, restaurant_id, dish_type) values ('Original Pretzel & Soda Combo', 'Every single pretzel is an original, and freshly baked to raise the standard of snacking. It’s the pretzel that started it all — made from five simple, fresh ingredients. Pick a pretzel up — it’s sure to return the favor!', '5.79', 1, 5);


insert into menu_item (title, description, price, restaurant_id, dish_type) values ('BLIMPIE BEST', 'SLOW-CURED HAM, SALAMI, CAPICOLA, PROSCIUTTINI, PROVOLONE MADE THE BLIMPIE® WAY WITH TOMATOES, LETTUCE, ONION, VINEGAR, OIL AND OREGANO', '5.29', 2, 2);


insert into menu_item (title, description, price, restaurant_id, dish_type) values ('Sausage and Cheese Stromboli', 'A Stromboli with sausage and cheese', '4.95', 3, 2);